...
func (c *thirdpartyClient) Login(username, password string) (string, error) {
	...
	c.logger.Debug("Login", log.Any("auths", auths), log.Any("user", username))
...
func (c *thirdpartyClient) Request(req *http.Request, res interface{}) error {
	...
	c.logger.Debug("thirdpary: response status code", log.Any("code", resp.StatusCode))
	if res != nil {
		resbody, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			return err
		}
		c.logger.Debug("thirdpary: response body", log.Any("body", string(resbody)))
