...
func WatchSecret(client kubernetes.Interface, namespace, labelSelector string, stop <-chan struct{}, p *AuthProcessor) error {
	...
	factory.Core().V1().Secrets().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			if selector.Matches(labels.Set(secret.Labels)) {
				if err := p.LoadFromSecret(nil, secret.Data); err != nil {
					logger.Error(fmt.Sprintf(`load config from secret "%s/%s" error`, secret.Namespace, secret.Name), log.String("func", "WatchSecret"))
				}
			}
		},
		UpdateFunc: func(old, new interface{}) {
			...
			if err != nil {
				logger.Error(fmt.Sprintf(`update config from secret "%s/%s" error`, secretNew.Namespace, secretNew.Name), log.String("func", "WatchSecret"))
			}
		},
		DeleteFunc: func(obj interface{}) {
			...
			if selector.Matches(labels.Set(secret.Labels)) {
				if err := p.LoadFromSecret(secret.Data, nil); err != nil {
					logger.Error(fmt.Sprintf(`remove config from secret "%s/%s" error`, secret.Namespace, secret.Name), log.String("func", "WatchSecret"))
				}
			}
		},
	})
