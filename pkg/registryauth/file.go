...
func readConfigFile(path string, p *AuthProcessor) error {
	...
	if err != nil {
		logger.Error(fmt.Sprintf(`read config file from "%s" error: %v`, path, err), log.String("func", "readConfigFile"))
		return err
	}
	if err := p.LoadFromFile(data); err != nil {
		logger.Error(fmt.Sprintf(`load config file from "%s" error: %v`, path, err), log.String("func", "readConfigFile"))
	}
	return err
}
